...
init();
  ...
  function addTableData(){
    ...
    let keys = ["name_customer", "cust_number", "invoice_id", "total_open_amount", "due_in_date_1", "delay_predicted", "notes"];
    let tbody = "<tbody>"
    list.forEach((row) => {
...
function checkDueDate(){
  ...
  // console.log(today);
  var dueDates = document.querySelectorAll(".due_in_date_1");
...
const add = () => {
  ...
  const due_in_date_1 = document.getElementById("dueDateInp").value;
  const notes =   document.getElementById("notesInp").value;
  console.log(due_in_date_1);
  if(name_customer == "" || cust_number == "" || invoice_id == "" || total_open_amount == "" || due_in_date_1 == ""){
    snackbar();
  } else {

  

  fetch(`/H2HBABBA2594/add?name_customer=${name_customer}&cust_number=${cust_number}&invoice_id=${invoice_id}&total_open_amount=${total_open_amount}&due_in_date_1=${due_in_date_1}&notes=${notes}`,
  {
    method: 'POST'
  }
  ).then(function(){
...
console.log(total_open_amount, notes);
    ...
    fetch(`/H2HBABBA2594/edit?invoice_id=${invoice_id}&total_open_amount=${total_open_amount}&notes=${notes}`, {
      method: 'POST'
    }
    ).then(function(){
      selectedRows = [];
...
const deleteData = () => {
  selectedRows.forEach((ele) => {
     ...
     fetch(`/H2HBABBA2594/delete?invoice_id=${invoice_id}`,
     {
      method: 'POST'
    }
     ).then(function(){
